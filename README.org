...
pixi run format-all
pixi run build examples/ArrayTester pico
pixi run upload examples/ArrayTester pico /dev/ttyACM0
pixi run monitor /dev/ttyACM0 115200
pixi run set-version -- 1.2.1
#+END_SRC
...
python3 tools/pio_task.py upload --example examples/ArrayTester --env pico --port /dev/ttyACM0
#+END_SRC
